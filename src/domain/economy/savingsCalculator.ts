...
export function checkBSULimits(account: SavingsAccount, year: number): BSUStatus {
  ...
  const estimatedYearlyContrib = account.monthlyContribution * 12

  const remainingYearlyQuota = Math.max(0, BSU_MAX_YEARLY - estimatedYearlyContrib)
  ...
  return {
    currentBalance,
    yearlyContributionSoFar: estimatedYearlyContrib, // monthlyContribution × 12
